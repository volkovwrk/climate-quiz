  ...
  if (window.ymaps && $("map")) {
    ymaps.ready(() => {
      state.map = new ymaps.Map(
        "map",
        {
          // Стартуем с более «широкого» обзора, чтобы в невидимой рамке
          // оказалась почти вся Земля (Камчатка, Антарктида и т.п.).
          center: [0, 0],
          zoom: 1,
          controls: [],
        },
        {
          ...
          // Ограничиваем карту стартовой областью (при center=[0,0], zoom=1).
          // Пользователь её не видит, потому что сразу после инициализации
          // мы меняем центр и масштаб, но ограничитель остаётся от широкого вида.
          restrictMapArea: true,
        }
      );

      // Сразу после инициализации возвращаем привычный вид карты для пользователя.
      state.map.setCenter([20, 0], 2);
